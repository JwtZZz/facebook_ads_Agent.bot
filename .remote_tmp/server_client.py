...
def _create_adset_with_app_support(
    self,
    campaign_id: str,
    name: str,
    ...
    if budget_amount_usd > 0:
        budget_key = "lifetime_budget" if budget_type == "lifetime" else "daily_budget"
        body[budget_key] = int(budget_amount_usd * 100)

    if application_id and object_store_url:
        body["promoted_object"] = {
...
def _create_campaign_with_budget_mode(
    self,
    name: str,
    budget_amount_usd: float = None,
    budget_type: str = "daily",
    use_campaign_budget: bool = True,
    ...
    if budget_amount_usd and use_campaign_budget:
        budget_key = "lifetime_budget" if budget_type == "lifetime" else "daily_budget"
        body[budget_key] = int(budget_amount_usd * 100)
        body["bid_strategy"] = "LOWEST_COST_WITHOUT_CAP"
    return self._req("POST", f"{self.cfg.account}/campaigns", body=body)["id"]
...
def _create_video_creative_with_app_support(
    self,
    name: str,
    video_id: str,
    landing_url: str,
    ...
    if not thumbnail_url:
        thumbnail_url = self.get_video_thumbnail(video_id)
    destination_url = object_store_url or landing_url
    cta_value = {"link": destination_url}
    if application_id:
        cta_value["application"] = application_id
    video_data: dict = {
        "video_id": video_id,
        "message": message,
        "title": title,
...
def _create_image_creative_with_app_support(
    self,
    name: str,
    image_hash: str,
    landing_url: str,
...
FBClient.create_campaign = _create_campaign_with_budget_mode
FBClient.create_adset = _create_adset_with_app_support
FBClient.create_video_creative = _create_video_creative_with_app_support
FBClient.create_image_creative = _create_image_creative_with_app_support
